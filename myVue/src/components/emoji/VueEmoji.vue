...
<script>
import ns from "./lib/js/jquery.nanoscroller";
// import ns from "./lib/js/jquery.nanoscroller";
import Vue from "vue";
import "./lib/css/emoji.css";
import "./lib/css/nanoscroller.css";
// import "./lib/css/nanoscroller.css";
...
export default {
  ...
  watch: {
    value: function(newVal) {
      Vue.nextTick(() => {
        if (this.transformEmoji) {
          this.transformEmoji = false;
  ...
  methods: {
    ...
    initEmojiPlugin() {
      ...
      Vue.nextTick(() => {
        ...
        ).style.width = `${Number(this.width) + 50}px`;
      });
    }
  },
  mounted: function() {
